...
/**
  ...
  *
  * spark-submit --class spark_hive_nsms --deploy-mode client --num-executors 2 --executor-memory 2g --executor-cores 2 --driver-memory 5g --master yarn /data2/gsj/ql_etl_nsms.jar
  */
object spark_hive_nsms {
...
//    val year =args(0).toString
    ...
    val conf = new SparkConf().setAppName("spark_hive_nsms")//.setMaster("local")
    ...
    val schemaString = "user_id,addtime,calltime,content,type,phone,querytime,device_id,device_id_type,mac,idfa,imei,name,count"
...
//    val json_rdd = sc.textFile("D:\\software\\ql_etl\\data\\nsms.json").map(x=>x.replaceAll("\\\\r\\\\n"," ").replaceAll("\\\\n"," ").replaceAll("\\\\r"," "))
//    val mobile = sqlContext.read.schema(schema).json(json_rdd)
//    mobile.show()


//    for (year <- years){
    val json_rdd = sc.textFile("/datahouse/ods/topic/biz_microsite_grabsms/"+date_etl+"/*").map(x=>x.replaceAll("\\\\r\\\\n"," ").replaceAll("\\\\n"," ").replaceAll("\\\\r"," "))
    .map(x=>originToJson(x))
//    val json_rdd = sc.textFile("/datahouse/ods/mongo/nsms/nsms-20170905.json").map(x=>x.replaceAll("\\\\r\\\\n"," ").replaceAll("\\\\n"," ").replaceAll("\\\\r"," "))
    val mobile = sqlContext.read.schema(schema).json(json_rdd)
    ...
    val mobile_select = mobile.select(
      ...
      regexp_replace(col("content"),"\\`","_").alias("content"),
    ...
    if(fs.exists(new org.apache.hadoop.fs.Path("/datahouse/ods/mongo/nsms_tmp/")))
      fs.delete(new org.apache.hadoop.fs.Path("/datahouse/ods/mongo/nsms_tmp/"),true)

    val expr = concat_ws("`", mobile_select.columns.map(col): _*)
//      mobile_select.na.fill("NULL").filter($"user_id"!=="NULL").select(expr).repartition(1).map(_.getString(0)).saveAsTextFile("/datahouse/ods/mongo/nsms_deal/nsms-"+year)
//      println("year:"+year+" is done==============================")

    mobile_select.na.fill("NULL").filter($"user_id"!=="NULL").select(expr).repartition(1).map(_.getString(0)).saveAsTextFile("/datahouse/ods/mongo/nsms_tmp/")

    if(fs.exists(new org.apache.hadoop.fs.Path("/user/hive/warehouse/dw_qlml.db/nsms_tmp/part-00000.snappy"))){
      fs.delete(new org.apache.hadoop.fs.Path("/user/hive/warehouse/dw_qlml.db/nsms_tmp/part-00000.snappy"),true)
    }

    fs.rename(new org.apache.hadoop.fs.Path("/datahouse/ods/mongo/nsms_tmp/part-00000.snappy"),new org.apache.hadoop.fs.Path("/user/hive/warehouse/dw_qlml.db/nsms_tmp/part-00000.snappy"))
