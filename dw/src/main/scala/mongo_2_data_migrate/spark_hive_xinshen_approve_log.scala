...
/**
  ...
  *
  * spark-submit --class mongo_2_data_migrate.spark_hive_xinshen_approve_log --deploy-mode client --num-executors 1 --executor-memory 2g --executor-cores 1 --driver-memory 5g --master yarn /data2/gsj/ql_etl.jar
  */
object spark_hive_xinshen_approve_log {
  def main(args: Array[String]) {
    ...
    println("xinshen_approve_log=========================================================date_etl:"+date_etl)

    val conf = new SparkConf().setAppName("spark_hive_xinshen_approve_log")//.setMaster("local")
    ...
    val table_schema_string = "xinshen_approve_log:user_id,borrow_nid,addtime,updatetime,operater_id,operater_name,code,memo,before_key,before_value,after_key,after_value"

    val table_name = table_schema_string.split(":")(0)
    val schemaString = table_schema_string.split(":")(1)
    val schema =StructType(schemaString.split(",").map(fieldName => StructField(fieldName, StringType, true)))
    ///datahouse/ods/mongo/xinshen_approve_log/xinshen_approve_log-201707.json
    val json_rdd = sc.textFile("/datahouse/ods/mongo/xinshen_approve_log/xinshen_approve_log-"+date_etl+".json").map(x=>x.replaceAll("\\\\r\\\\n"," ").replaceAll("\\\\n"," ").replaceAll("\\\\r"," "))
    val xinshen_approve_log = sqlContext.read.schema(schema).json(json_rdd)


    val xinshen_approve_log_select = xinshen_approve_log.select(xinshen_approve_log.columns.map(c => regexp_replace(col(c),"\\`","_").alias(c)):_*)

    val expr = concat_ws("`", xinshen_approve_log_select.columns.map(col): _*)
    ...
    xinshen_approve_log_select.na.fill("NULL").select(expr).repartition(1).map(_.getString(0)).saveAsTextFile("/tmp/"+table_name+"_tmp/")
