...
object spark_hive_ncontacts {
  def main(args: Array[String]) {
    ...
    if (args.length == 1) {
      ...
      date_etl = DateTime.now().withTimeAtStartOfDay().plusDays(-1).toString("yyyyMMdd")
    }

    val year = args(0).toString


    val conf = new SparkConf().setAppName("spark_hive_ncontacts")
    //.setMaster("local")
    ...
    //    for (year <- years){
    val json_rdd = sc.textFile("/datahouse/ods/mongo/ncontacts/ncontacts-" + date_etl + ".json").map(x => x.replaceAll("\\\\r\\\\n", " ").replaceAll("\\\\n", " ").replaceAll("\\\\r", " "))
