...
//    val year =args(0).toString
    ...
    //新增字段  imsi,record_time,ac,fromChannel
    val schemaString = "user_id,addtime,count,calltime,type,duration,phone,name,querytime,device_id,device_id_type,mac,idfa,imei,imsi,record_time,ac,fromChannel"
...
//    for (year <- years){
      val json_rdd = sc.textFile("/datahouse/ods/topic/biz_microsite_grabcallrds/"+date_etl+"/*").map(x=>x.replaceAll("\\\\r\\\\n"," ").replaceAll("\\\\n"," ").replaceAll("\\\\r"," "))
