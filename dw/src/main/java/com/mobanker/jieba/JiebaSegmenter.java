...
public class JiebaSegmenter {
    ...
    public List<SegToken> process(String paragraph, com.huaban.analysis.jieba.JiebaSegmenter.SegMode mode) {
        List<SegToken> tokens = new ArrayList<SegToken>();
        StringBuilder sb = new StringBuilder();
        int offset = 0;
        for (int i = 0; i < paragraph.length(); ++i) {
            char ch = com.huaban.analysis.jieba.CharacterUtil.regularize(paragraph.charAt(i));
            if (CharacterUtil.ccFind(ch))
                sb.append(ch);
            else {
                if (sb.length() > 0) {
                    // process
                    if (mode == com.huaban.analysis.jieba.JiebaSegmenter.SegMode.SEARCH) {
