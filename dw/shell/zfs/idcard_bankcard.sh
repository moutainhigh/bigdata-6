...
#spark etl
su hdfs -c "spark-submit --class spark_ocr_identify --deploy-mode client --num-executors 1 --executor-memory 2g --executor-cores 1 --driver-memory 5g --master yarn /data2/zfs/ql_etl.jar 2 1"
...
from rca.yyd_rca_idcard_validity_tmp;"

echo "`date "+%Y-%m-%d %H:%M:%S"`  yyd_rca_idcard_validity_tmp To hive has done" >> $logfile
